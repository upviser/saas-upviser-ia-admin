            ...
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/suscribe`, {
                ...
                .then(async (response) => {
                  if (response.status === 'Processed') {
                    ...
                    await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/shop-login-admin`, { state: true, subscription: true, dateSubscription: new Date(), plan: plan?.name, textAI: plan?.name === 'Plan Esencial' ? 0 : plan?.name === 'Plan Avanzado' ? 80 : plan?.name === 'Plan Profesional' ? 160 : 0, imagesAI: plan?.name === 'Plan Esencial' ? 0 : plan?.name === 'Plan Avanzado' ? 40 : plan?.name === 'Plan Profesional' ? 80 : 0, videosAI: plan?.name === 'Plan Esencial' ? 0 : plan?.name === 'Plan Avanzado' ? 0 : plan?.name === 'Plan Profesional' ? 20 : 0, conversationsAI: plan?.name === 'Plan Esencial' ? 100 : plan?.name === 'Plan Avanzado' ? 200 : plan?.name === 'Plan Profesional' ? 400 : 0, emails: plan?.name === 'Plan Esencial' ? 1000 : plan?.name === 'Plan Avanzado' ? 2000 : plan?.name === 'Plan Profesional' ? 4000 : 0 })
                    setLoading(false)
                    resolve();
                  } else {
                    let currentClient = clientRef.current
